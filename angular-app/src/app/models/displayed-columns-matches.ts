export class DisplayedColumnsForMatches {
    ...
    time: [on: boolean, order: number] = [true, 1];
    day: [on: boolean, order: number] = [true, 2];
    description: [on: boolean, order: number] = [true, 3];
    cup: [on: boolean, order: number] = [true, 4];
    number: [on: boolean, order: number] = [true, 5];
    more: [on: boolean, order: number] = [true, 6];

    public get displayedColumns(): string[] {
        var colsToDisplay: string[] = [];

        if (this.date[0] == true) { 
            colsToDisplay[this.date[1]] = "date"
            colsToDisplay[this.time[1]] = "time"
        }
